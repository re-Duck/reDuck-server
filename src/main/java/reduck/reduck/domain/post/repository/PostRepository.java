...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query("select p from Post p join fetch p.user where p.postType in(:postType) and  (select post.id from Post post where post.postOriginId = :postOriginId) > p.id order by p.id desc")
